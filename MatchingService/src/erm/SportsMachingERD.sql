...
DROP TRIGGER TRI_GAMES_GAME_ID;
DROP TRIGGER TRI_GROUNDS_GROUND_ID;
DROP TRIGGER TRI_MATCHING_MATCHING_ID;
DROP TRIGGER TRI_WINORLOSE_WL_ID;
...
DROP SEQUENCE SEQ_GAMES_GAME_ID;
DROP SEQUENCE SEQ_GROUNDS_GROUND_ID;
DROP SEQUENCE SEQ_MATCHING_MATCHING_ID;
DROP SEQUENCE SEQ_WINORLOSE_WL_ID;
...
CREATE SEQUENCE SEQ_GAMES_GAME_ID INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_GROUNDS_GROUND_ID INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_MATCHING_MATCHING_ID INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_WINORLOSE_WL_ID INCREMENT BY 1 START WITH 1;
...
CREATE OR REPLACE TRIGGER TRI_GAMES_GAME_ID BEFORE INSERT ON GAMES
FOR EACH ROW
BEGIN
	SELECT SEQ_GAMES_GAME_ID.nextval
	INTO :new.GAME_ID
	FROM dual;
END;
...
CREATE OR REPLACE TRIGGER TRI_GROUNDS_GROUND_ID BEFORE INSERT ON GROUNDS
FOR EACH ROW
BEGIN
	SELECT SEQ_GROUNDS_GROUND_ID.nextval
	INTO :new.GROUND_ID
	FROM dual;
END;
...
CREATE OR REPLACE TRIGGER TRI_MATCHING_MATCHING_ID BEFORE INSERT ON MATCHING
FOR EACH ROW
BEGIN
	SELECT SEQ_MATCHING_MATCHING_ID.nextval
	INTO :new.MATCHING_ID
	FROM dual;
END;

/

CREATE OR REPLACE TRIGGER TRI_WINORLOSE_WL_ID BEFORE INSERT ON WINORLOSE
FOR EACH ROW
BEGIN
	SELECT SEQ_WINORLOSE_WL_ID.nextval
	INTO :new.WL_ID
	FROM dual;
END;
...
COMMENT ON COLUMN RECORD.FB_TOTAL IS '축구_승';
COMMENT ON COLUMN RECORD.FB_WIN IS '축구_패';
COMMENT ON COLUMN RECORD.BB_TOTAL IS '농구_승';
COMMENT ON COLUMN RECORD.BB_WIN IS '농구_패';
COMMENT ON COLUMN RECORD.PP_TOTAL IS '배드민턴_승';
COMMENT ON COLUMN RECORD.PP_WIN IS '배드민턴_패';
COMMENT ON COLUMN RECORD.BL_TOTAL IS '볼링_승';
COMMENT ON COLUMN RECORD.BL_WIN IS '볼링_패';
