	  ...
	  <script type="text/javascript">
        ...
        /* 	 $.ajax({
        		 ...
        		 success:function(resp){
        			 var list=resp.boardlist
        			 $.each(resp.list,function(index,value){
        				 $('#tab').append(
        			  				"<tr>"+
        			  				"<td class='"'cl'"' >"+list[index].boardNum +"</td>"+
        			  				"<td class='"'cl'"'>"+ list[index].category+"</td>"+       			  				
        			  				"<td class='"'cl'"'>"+ list[index].title +"</td>"+
        			  				"<td c    lass='"'cl'"'>"+ list[index].user_Id +"</td>"+
        			  				"<td class='"'cl'"'>"+ list[index].inputDate +"</td>"+
        			  				"</tr>"
        		  				);//append
...
color: #333333;
	background-color: #E5E5D8;
	 ...
	 table tfoot td,  table tfoot th{
       ...
       <h1 class="page-title">게시글 보기 - <span class="fw-semi-bold">목록</span></h1> 
       <div id="table">
      	<form>
        <table id="tab">
        <thead>
       ...
       <s:iterator value="boardlist">
        ...
        <td ><s:property value="boardNum" /></td>
         <td ><s:if test="category==0">
         	잡담
         </s:if><s:elseif test="category==1">리뷰</s:elseif>
         <s:else>정보</s:else>
         
         
         </td>
         <td ><s:a action="read"><s:param value="%{boardNum}" name="num"/><s:property value="title"/></s:a></td>
